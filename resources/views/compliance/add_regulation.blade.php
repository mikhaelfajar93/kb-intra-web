                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            <h4 class="card-title">Add Regulation Compliance</h4>
                        </div>
                    </div>
                    <div class="card-body">
                        @if (session('status'))
                        ...
                        @if ($errors->any())
                            ...
                            @endforeach
                        @endif
                        <form role="form" action="/comp-add-regulation" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group">
                                <label class="form-label" for="exampleInputText1">Title </label>
                                <input type="text" class="form-control" id="title_regulation_comp"
                                    name="title_regulation_comp" value="{{ old('title_regulation_comp') }}"
                                    placeholder="Enter Title">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="exampleFormControlTextarea1">Details</label>
                                <textarea class="form-control" id="detail_regulation_comp" name="detail_regulation_comp" rows="5">{{ old('detail_regulation_comp') }}</textarea>
                            </div>
                            <div class="form-group">
                                <label for="customFile1" class="form-label custom-file-input">Upload file</label>
                                <input class="form-control" type="file" id="file_regulation_comp"
                                    name="file_regulation_comp[]" multiple>
                            </div>
