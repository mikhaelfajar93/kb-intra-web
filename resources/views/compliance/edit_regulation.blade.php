...
@section('regulation_comp_active', 'active')
@section('content')
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            <h4 class="card-title">Edit Regulation Compliance</h4>
                        </div>
                    </div>
                    <div class="card-body">
                        @if ($errors->any())
                            ...
                            @endforeach
                        @endif
                        <form role="form" action="{{ route('comp.update_regulations', $regulations->id) }}"
                            method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label class="form-label" for="exampleInputText1">Title Announcement </label>
                                <input type="text" class="form-control" id="title_regulation_comp"
                                    name="title_regulation_comp" value="{{ $regulations->title }}"
                                    placeholder="Enter Title">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="exampleFormControlTextarea1">Details</label>
                                <textarea class="form-control" id="detail_regulation_comp" name="detail_regulation_comp" rows="5">{{ $regulations->details }}</textarea>
                            </div>
                            @if ($count_file_regulation != 0)
                                <div class="form-group">
                                    <label for="customFile1" class="form-label custom-file-input">Existing file</label>
                                    @foreach ($file_regulation as $fileregulation)
                                                ...
                                                <div class="d-flex align-items-center p-4">
                                                    <div class="w-100">
                                                        <div class="d-flex">
                                                            ...
                                                            <span
                                                                class="ms-2 newsfilename">{{ $fileregulation->filename }}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            @else
                                <div class="form-group">
                                    ...
                                    <input class="form-control" type="file" id="file_regulation" name="file_regulation[]"
                                        multiple>
                                </div>
                            @endif
                            <div class="mt-3 mt-sm-0">
                                <a href="/regulation_compliance" role="button" data-toggle="tooltip" title="Back"
