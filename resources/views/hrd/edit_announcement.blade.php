...
@section('announcement_hrd_active', 'active')
@section('content')
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            <h4 class="card-title">Edit Announcement HRD</h4>
                        </div>
                    </div>
                    <div class="card-body">
                        @if ($errors->any())
                            ...
                            @endforeach
                        @endif
                        <form role="form" action="{{ route('hrd.update_announcements', $announcements->id) }}"
                            method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label class="form-label" for="exampleInputText1">Title Announcement </label>
                                <input type="text" class="form-control" id="title_announcement_hrd"
                                    name="title_announcement_hrd" value="{{ $announcements->title }}"
                                    placeholder="Enter Title">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="exampleFormControlTextarea1">Details</label>
                                <textarea class="form-control" id="detail_announcement_hrd" name="detail_announcement_hrd" rows="5">{{ $announcements->details }}</textarea>
                            </div>
