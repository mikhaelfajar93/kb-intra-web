                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            <h4 class="card-title">Add Announcement Legal</h4>
                        </div>
                    </div>
                    <div class="card-body">
                        @if (session('status'))
                        ...
                        @if ($errors->any())
                            ...
                            @endforeach
                        @endif
                        <form role="form" action="/legal-add-announcement" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group">
                                <label class="form-label" for="exampleInputText1">Title </label>
                                <input type="text" class="form-control" id="title_announ_legal" name="title_announ_legal"
                                    value="{{ old('title_announ_legal') }}" placeholder="Enter Title">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="exampleFormControlTextarea1">Details</label>
                                <textarea class="form-control" id="detail_announ_legal" name="detail_announ_legal" rows="5">{{ old('detail_announ_legal') }}</textarea>
                            </div>
                            <div class="form-group">
                                <label for="customFile1" class="form-label custom-file-input">Upload file</label>
                                <input class="form-control" type="file" id="file_announ_legal" name="file_announ_legal[]"
                                    multiple>
                            </div>
                            <div class="mt-3 mt-sm-0">
                                <a href="/announ_legal" role="button" data-toggle="tooltip" title="Back"
