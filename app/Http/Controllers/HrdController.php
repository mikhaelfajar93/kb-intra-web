...
class HrdController extends Controller
{
    public function page_dashboard_announcement()
    {
        ...
        if ($role == 'HRD') {
            $list_announ = Announcement::where('roleby', '=', 'HRD')->get();
        } else {
            $list_announ = Announcement::where('roleby', 'HRD')
                ->where('status', 1)
                ->get();
        }

        return view('hrd.home_announcement', compact('username', 'role', 'list_announ'));
    }
    ...
    public function page_dashboard_regulation()
    {
        ...
        if ($role == 'HRD') {
            $list_regulation = Regulation::where('roleby', '=', 'HRD')->get();
        } else {
            $list_regulation = Regulation::where('roleby', 'HRD')
                ->where('status', 1)
                ->get();
        }

        return view('hrd.home_regulation', compact('username', 'role', 'list_regulation'));
    }
    ...
    public function page_input_announcement()
    {
        ...
        return view('hrd.add_announcement', compact('username', 'role'));
    }
    ...
    public function page_input_regulation()
    {
        ...
        return view('hrd.add_regulation', compact('username', 'role'));
    }
    ...
    public function input_regulation(Request $request)
    {
        ...
        $regulations->title = $request->title_regulation_hrd;
        $regulations->details = $request->detail_regulation_hrd;
        ...
        $validator = Validator::make($request->all(), [
            'file_regulation_hrd.*' => 'file|mimes:jpg,jpeg,png,pdf,doc,docx,xls,xlsx,ppt,pptx||max:' . $maxFileSize,
        ]);
        ...
        $file_regulation = [];
        if ($request->hasFile('file_regulation_hrd')) {
            foreach ($request->file('file_regulation_hrd') as $value) {
    ...
    public function input_announcement(Request $request)
    {
        ...
        $annoucements->title = $request->title_announ_hrd;
        $annoucements->details = $request->detail_announ_hrd;
        ...
        $maxFileSize = 12 * 1024; //maxium size file upload
        $validator = Validator::make($request->all(), [
            'file_announ_hrd.*' => 'file|mimes:jpg,jpeg,png,pdf,doc,docx,xls,xlsx,ppt,pptx||max:' . $maxFileSize,
        ]);
        ...
        $file_announcement = [];
        if ($request->hasFile('file_announ_hrd')) {
            foreach ($request->file('file_announ_hrd') as $value) {
                $filename = $value->getClientOriginalName();
    ...
    public function edit_regulation($idregulation)
    {
        ...
        if ($regulations->status == 0) {
            return view('hrd.edit_regulation', compact('username', 'role', 'regulations', 'file_regulation', 'count_file_regulation'));
        } else {
            ...
            return redirect()->route('regulation_hrd');
        }
    ...
    public function update_regulation(Request $request, $idregulation)
    {
        ...
        $regulations->title = $request->title_news_hrd;
        $regulations->details = $request->detail_news_hrd;
        $regulations->uploadby = $username;
        $regulations->roleby = $role;
        $regulations->updated_at = $today;
        $regulations->update();
    ...
    public function edit_announcement($idannouncement)
    {
        ...
        $list_announcement = Announcement::where('roleby', '=', 'HRD')->get();

        if ($announcements->status == 0) {
            return view('hrd.edit_announcement', compact('username', 'role', 'announcements', 'file_announcement', 'count_file_announcement'));
        } else {
            session(['username' => $username, 'role' => $role]);
            session()->flash('status', 'You cannot update regulation because the regulation has been posted');
            // return redirect()->route('regulation_compliance')->with(compact('username', 'role', 'list_regulation'));
            return redirect()->route('announ_hrd');
        }
    ...
    public function update_announcement(Request $request, $idannouncement)
    {
        ...
        $announcements = Announcement::findOrFail($idannouncement);
        // dd($request->title_news_comp);
        $announcements->title = $request->title_announcement_hrd;
        $announcements->details = $request->detail_announcement_hrd;
    ...
    public function view_regulation($idregulation)
    {
        ...
        return view('hrd.view_regulation', compact('username', 'role', 'regulations', 'file_regulation', 'count_file_regulation'));
    }
    ...
    public function view_announcement($idannouncement)
    {
        ...
        return view('hrd.view_announcement', compact('username', 'role', 'announcements', 'file_announcement', 'count_file_announcement'));
    }
